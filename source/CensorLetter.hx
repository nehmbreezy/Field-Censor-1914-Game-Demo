 ...
 * @author Alexander
 * 
 * This class contains all the code for drawing/detecting censor marks on text.
 * 
 */

class CensorLetter extends FlxGroup
{
	...
	public function new(startX:Int, startY:Int, text: String, cardGraphic:Dynamic, templateGraphic:Dynamic, approval:Bool, missing:Bool, killed:Bool ) : Void
	{
		super(360);
		...
		lineStyle1 = { color : FlxColor.MAROON, thickness : 27.5, pixelHinting : false};
		...
		canvas.x = card.x;
			...
			for (i in 0...buttons.length) 
			{
				...
				if (buttons[i].clicked == true && buttons[i].alreadyDone != true)
				{
					...
					Registry.log.text = buttons[i].text;
	...
	{
		feedback();
		FlxTween.tween(card, { x:2050, y:130 }, 0.6, { ease: FlxEase.quadIn, complete:beginDestroy});
	}
	
	private function beginDestroy(flxtween:FlxTween):Void
	{
